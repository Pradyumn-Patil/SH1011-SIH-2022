...
<?php
    define('menu','menu');
    $menu =  $_REQUEST['menu'];  //Output: myquery
?>
<nav class="navbar">
    <ul class="navbar-nav">
      <li class="logo">
        <a href="index.php?menu=welcome" class="nav-link" id="menu-garuda">
          <span class="link-text logo-text">GARUDA</span>
      ...
      <li class="nav-item">
        <a href="index.php?menu=vim" class="nav-link" id="vulnerability-index-map">
          ...
          <span class="link-text">Vulnerability Index Map</span>
        </a>
      </li>

      <li class="nav-item">
        <a href="index.php?menu=syd" class="nav-link" id="state-year-disaster">
          ...
          <span class="link-text">State Year Disaster</span>
        </a>
      </li>

      <li class="nav-item">
        <a href="https://house-recommendation.herokuapp.com" class="nav-link">
...
<?php
      if ($menu === "syd"){
?> 
      <div id='container-map'> <!-- Start of Map -->       
        <h2 id='title'> State Year wise Disaster </h2>
        <div id='description'>
          total number of Flooding events.
        </div>
        <div id='hazardtype'>
          <select name="disaster" id="disaster" onchange="drawMap()">
          <option value="EQ">Select disaster</option>
      ...
      <div id='container-bar-chart'>  <!-- Start of Bar Chart -->
...
<% }); %>
        ...
        <script type="text/javascript">
          ...
          let statedisasterURLBar = 'http://localhost/sih/geo%20json/state_yearwise.json'

          let statediasterYearData
          let countyData
          /*global $, d3v3, _ */
          function drawBarChart(id, type) {
            var yzdata
            if (type === 'taluka') {
              yzdata = GetYearwiseData(id, educationData)
            }
            else {
              yzdata = GetYearwiseData(id, statediasterYearData)
            }
          ...
          async function fetchData(url) {
            ...
            statediasterYearData = await response.json();  // Filter Data for a fips 
            drawBarChart('Maharashtra', "state")
          }

          fetchData(statedisasterURLBar)


          // filtering data for fips for converting year wise data for stacking in group bar chart 
          function GetYearwiseData(fipscode, Data) {
            var localdata = Data.filter(function (fipsdata) {
 ...
 ?>
      <div> <!-- Start of Zoom-->
        <h2 id='title'> Vulnerability Index </h2>
    
        <div id='hazardtype'>
          <select name="zoomdisaster" id="zoomdisaster" onchange="drawZoomMap()">
            <option value="EQ">earthquake</option>
            <option value="HWV">Heatwave</option>
            <option value="CYC">Cyclone</option>
            <option value="FL">Flood</option>
            <option value="UFL">urban flood</option>
            <option value="HWN">Heatwave</option>
            <option value="TSU">Tsunami</option>
            <option value="CHEM">Chemical hazard</option>
            <option value="NUC">Nuclear Hazard</option>
            <option value="BIO">Biological Hazard</option>
        ...
        <div id='hazardtype'>
          <select name="disaster" id="disaster">
            <option value="EQ">ALL states</option>
          </select>
        </div>
        <svg id='canvas' width="950" height="600">
          <g> </g>
      ...
      </div id='container-zoom'> <!--  End  of Zoom -->
